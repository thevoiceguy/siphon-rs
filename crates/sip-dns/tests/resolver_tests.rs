...
async fn resolve_numeric_ipv4() {
    ...
    assert_eq!(targets[0].host.as_str(), "192.168.1.1");
...
async fn resolve_numeric_ipv6() {
    ...
    assert_eq!(targets.len(), 1);
    assert_eq!(targets[0].host.as_str(), "::1");
    assert_eq!(targets[0].port, 5060);
}
...
async fn static_resolver_integration() {
    let targets = vec![
        DnsTarget::new("sip1.example.com", 5060, Transport::Udp),
        DnsTarget::new("sip2.example.com", 5060, Transport::Tcp).with_priority(10),
        DnsTarget::new("sip3.example.com", 5060, Transport::Tls).with_priority(20),
    ];
    ...
    assert_eq!(result[0].host.as_str(), "sip1.example.com");
    assert_eq!(result[1].host.as_str(), "sip2.example.com");
    assert_eq!(result[2].host.as_str(), "sip3.example.com");
...
async fn failover_scenario() {
    ...
    let targets = vec![
        DnsTarget::new("primary.example.com", 5060, Transport::Udp).with_priority(10),
        DnsTarget::new("secondary.example.com", 5060, Transport::Udp).with_priority(20),
        DnsTarget::new("tertiary.example.com", 5060, Transport::Udp).with_priority(30),
    ];
    ...
    assert_eq!(result[0].host.as_str(), "primary.example.com");
    assert_eq!(result[0].priority, 10);
    assert_eq!(result[1].host.as_str(), "secondary.example.com");
    assert_eq!(result[1].priority, 20);
    assert_eq!(result[2].host.as_str(), "tertiary.example.com");
...
async fn multiple_transports_for_failover() {
    ...
    let targets = vec![
        DnsTarget::new("example.com", 5061, Transport::Tls).with_priority(10),
        DnsTarget::new("example.com", 5060, Transport::Tcp).with_priority(20),
        DnsTarget::new("example.com", 5060, Transport::Udp).with_priority(30),
    ];
...
async fn websocket_transport_support() {
    ...
    let targets = vec![DnsTarget::new("ws.example.com", 80, Transport::Ws)];
    let resolver = StaticResolver::new(targets);
    ...
    let targets = vec![DnsTarget::new("wss.example.com", 443, Transport::Wss)];
    let resolver = StaticResolver::new(targets);
...
async fn dns_target_equality() {
    let target1 = DnsTarget::new("example.com", 5060, Transport::Udp);
    let target2 = DnsTarget::new("example.com", 5060, Transport::Udp);
    let target3 = DnsTarget::new("example.com", 5060, Transport::Tcp);
